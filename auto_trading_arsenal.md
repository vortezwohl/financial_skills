...
- [十、现实边界：哪些平台不支持 Agent 自动化](#十现实边界哪些平台不支持-agent-自动化)
- [快速集成建议](#快速集成建议)
...
## 十、现实边界：哪些平台不支持 Agent 自动化

> 不是所有金融平台都开放 API。以下梳理了两个最常被问到的平台——支付宝（蚂蚁财富）和 ZA Bank（众安银行）的自动化可行性。

### 支付宝 / 蚂蚁财富：❌ 不支持

| 维度 | 现状 |
|------|------|
| **个人 API** | ❌ **不存在**。不对个人开发者或第三方提供交易接口 |
| **持仓查询** | ❌ 无 API。只能手动在 APP 内查看 |
| **自动下单** | ❌ 不可行。支付场景受强安全管控 |
| **程序化调仓** | ❌ 无接口。公募基金程序化交易在国内属于灰色地带 |

**唯一官方自动化方案：「帮你投」**

| 特性 | 说明 |
|------|------|
| 运营方 | 蚂蚁集团（51%）+ Vanguard 先锋领航（49%）合资 |
| 模式 | 全权委托智能投顾 |
| 费率 | 约 0.5%/年（按日计提） |
| 调仓 | 每 90 天定期检测 + 不定期触发 |
| 起投 | 10 元 |
| **限制** | **封闭策略，不可自定义**；用户只能选择风险等级，无法编程干预 |

**蚂蚁数科 MCP（toB 专用）**

2025 年蚂蚁数科推出了「金融 MCP 服务」，但：
- 面向**持牌金融机构**（银行/券商/基金公司），不对个人开放
- 提供基金诊断、配置选品、行业建议等投顾工具
- 百宝箱 MCP 专区（2025.04）面向 AI 开发者，但不提供交易接口
- 2025.09 开放「三大AI助手」（投研/运营/内容），352 家金融机构接入，依然是 toB
...
| **API 自动化交易** | ❌ 未提供 |
| **投顾/自动调仓** | ❌ 无此类产品 |
| **加密交易** | ✅ APP 内手动交易 BTC/ETH（通过 HashKey Pro 后端集成） |

ZA Bank 是香港首家提供虚拟资产交易的数字银行，其加密交易能力来自与 HashKey Pro 的合作——但这属于 **B2B 机构级 API 集成**，用于支撑 APP 功能，而非开放给终端用户。

### 替代路径：如果想做 Agent 自动化交易，应该往哪走

| 想要实现 | 真正的可行路径 |
|----------|---------------|
| **获取基金数据** | AKShare (#15) / Tushare (#14) — 获取公募基金净值、排名、持仓等公开数据 |
| **A股自动下单** | 券商量化终端 — **QMT**（国金/华泰等）、**PTrade**（华泰/中信等）— 仅限场内 ETF/LOF |
| **美股自动下单** | Alpaca MCP (#5) / IB MCP (#10) — 支持 API 自动交易 |
| **加密自动下单** | Freqtrade (#35) / QuantDinger (#36) / CCXT — 支持主流交易所 |
| **AI Agent 分析** | 自建 MCP Server（参考 FinQ4Cn / A-Scope-Research）+ LangChain / AutoGen |
| **全自动基金调仓** | 持牌投顾机构全委托账户（盈米「且慢」、先锋领航「帮你投」等） |

### 底线

```
┌──────────────────────────────────────────────────────┐
│  支付宝/蚂蚁基金  ──→  封闭平台，无 Agent 接口          │
│  ZA Bank         ──→  封闭平台，无 Agent 接口          │
│                                                      │
│  真正能 Agent 自动化的路径：                            │
│  • A股/期货 → 券商 QMT/PTrade/CTP                     │
│  • 美股     → Alpaca / IBKR / Robinhood               │
│  • 加密     → Binance / OKX / CCXT                    │
│  • 基金分析 → AKShare + 自建 MCP（只看不交易）          │
└──────────────────────────────────────────────────────┘
```
